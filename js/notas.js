...
async function abrirAttachments() {
  ...
  document.getElementById('att-modal-subtitle').textContent =
    `Note: ${descifrar(notaActual.title_enc) || 'Untitled'}`
...
function crearAttCard(att) {
  ...
  nameEl.onblur = async () => {
    ...
    await db.from('note_attachments')
      .update({ display_name: `${nuevo}.${extOrig}` })
      .eq('id', att.id)
  }
  nameEl.onkeydown = e => {
...
async function subirAttachment(file) {
  ...
  // Auto-insert chip at cursor position
  if (attData) {
    await insertarChipEnEditor(attData)
...
async function insertarChipEnEditor(att) {
  ...
  // Restore saved cursor range or append at end
  if (_attCursorRange) {
    try {
      const sel = window.getSelection()
      sel.removeAllRanges()
...
async function crearChip(att) {
  ...
  if (isImage) {
    ...
    thumb.textContent = attIcono(ext)
  }
  ...
  chip.onclick = e => {
    ...
    document.querySelectorAll('.att-chip-menu.open').forEach(m => {
  ...
  // Drag events for repositioning within editor
...
async function obtenerStorageUsado() {
  ...
  const { data: atts } = await db.from('note_attachments').select('size_kb')
    .in('note_id', noteIds).eq('is_deleted', false)
...
// ---- Restore chips on note open ----
// Chips are saved as HTML inside nota-contenido — they restore automatically
// Just need to re-attach event listeners after loading
...
function restaurarChipsEnEditor() {
  ...
  editor.querySelectorAll('.att-chip').forEach(async chip => {
    ...
    const attPath = chip.dataset.attPath
    const attName = chip.dataset.attName
    const attExt = chip.dataset.attExt
    if (!attId) return

    // Re-fetch attachment data
    const { data: att } = await db.from('note_attachments')
      .select('*').eq('id', attId).single()
    if (!att) return
